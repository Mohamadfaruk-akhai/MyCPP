...
void closeAccountOperation:: closeAccount(int pid) //!< definition of closeAccount function which take 1 parameter to close Account of 								particular user
{
	       	int j; /**< declaration of j variable */
		int flag=0; /**< declaration of flag variable */
	      for(j=0;j<l;j++)
		{
		    	if(Pid[j]==pid) /*!condition to check user entered id match to Pid array or not*/
			{
			...
			if(flag==0) 
				...
				for(int k=j;k<l;k++)
				Pid[j]=Pid[j+1]; /*!shifting value of Pid array from j index to j+1*/
		    	}
}
